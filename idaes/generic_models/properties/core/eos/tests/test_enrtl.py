...
class TestStateBlockSymmetric(object):
    ...
    def test_common(self, model):
        ...
        assert str(model.state[1].Liq_relative_permittivity_solvent.expr) == (
            str(model.params.get_component(
    ...
    def test_G(self, model):
        ...
        assert str(model.state[1].Liq_G["C6H12", "H+"].expr) == str(
                (model.state[1].Liq_Y["Cl-"] *
                 ...
                 model.state[1].Liq_Y["OH-"] *
                 exp(-model.params.Liq.alpha["C6H12", "H+, OH-"] *
...
class TestProperties():
    ...
    def test_fug_phase_comp_eq(self, model):
        ...
        for p, c in model.state[0].phase_component_set:
            if c == "H2O":
                assert value(ENRTL.fug_phase_comp_eq(
                    model.state[0], p, c, "temp")) == pytest.approx(
                    self.FUG_H2O_eq*self.ACT_H2O*0.9, rel=1e-6)
            else:
                assert ENRTL.fug_phase_comp_eq(
    ...
    def test_fug_coeff_phase_comp_eq(self, model):
        ...
        for p, c in model.state[0].phase_component_set:
            if c == "H2O":
                assert value(ENRTL.fug_coeff_phase_comp_eq(
                    model.state[0], p, c, "temp")) == pytest.approx(
                    self.FUG_H2O_eq*self.ACT_H2O*0.9/101325, rel=1e-6)
            else:
